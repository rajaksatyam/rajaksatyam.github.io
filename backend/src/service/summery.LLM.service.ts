...
// Keep your PROMPT string exactly as it is.
const PROMPT = `...`; 

// Notice: I cleaned up the interface slightly to perfectly match your JSON prompt.
// Your prompt outputs "overallVerdict", but your interface had both "verdict" and "overallVerdict".
interface TranscriptionEntry {
  timestamp: string
  text: string
}
...
const analyzeWithGemini = async (cdnURL: string): Promise<Analysis> => {
  const response = await geminiAI.models.generateContent({
    // gemini-2.5-flash is the standard identifier
    model: "gemini-2.5-flash", 
    contents: `Analyze this content: ${cdnURL}`,
    config: {
      // 1. Move the prompt to systemInstruction so the model treats it as absolute law
      systemInstruction: PROMPT,
      
      // 2. Enable Google Search grounding
      tools: [{ googleSearch: {} }],
      
      // 3. Force native JSON output (No markdown backticks, guaranteed JSON syntax)
  ...
  // 4. Because of responseMimeType, we can safely parse without regex stripping
  const parsed: Analysis = JSON.parse(rawText)
  return parsed
}
...
export const analyzeVideo = async (cdnURL: string): Promise<Analysis> => {
  try {
    ...
    logger.warn(error, "Gemini error:")
    throw new Error(
      "LLM provider failed. Please check your API keys, the URL, or the system logs."
    )
  }
}